		...
		case ASSIGNOP_DIV:
					...
					else 
					{
						...
						else 
						{
							...
							if (strcmp(T->type_id,"+=")==0)
									T->kind=ASSIGNOP_PLUS;
							else if (strcmp(T->type_id,"-=")==0)
									T->kind=ASSIGNOP_MINUS;
							else if (strcmp(T->type_id,"*=")==0)
									T->kind=ASSIGNOP_STAR;
							else if (strcmp(T->type_id,"/=")==0)
									T->kind=ASSIGNOP_DIV;
						T->width=T->ptr[0]->width+T->ptr[1]->width+(T->type==CHAR?1:(T->type==DOUBLE?8:4));
						}
...
void array_dec(struct node *T)
{
	...
	while(T0)
	{
		if(T0->kind != ID)
		{
			array_pars[array_size++] = T0->ptr[1]->type_int;//记录第i维数组大小
...
//for条件行处理函数
void for_dec_line(struct node *T)
{
...
void for_dec(struct node *T)
{
    ...
    //strcpy(T->ptr[0]->Efalse, T->Snext);
    T->ptr[0]->offset = T->ptr[1]->offset = T->offset;
    for_dec_line(T->ptr[0]); //for循环条件，单独处理
    T->width = T->ptr[0]->width;
    //strcpy(T->ptr[1]->Snext, newLabel());
	T->isWhile = T->ptr[1]->isWhile = 1;
	//strcpy(T->ptr[1]->Ebreak, T->Snext);
...
{
    symbolTable.index = 0;
    //add_symbolTable("read", "", 0, INT, 'F', 4);
    //symbolTable.symbols[0].paramnum = 0; //read的形参个数
    //add_symbolTable("x", "", 1, INT, 'P', 12);
    //add_symbolTable("write", "", 0, INT, 'F', 4);
