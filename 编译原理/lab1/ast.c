...
{ //对抽象语法树的先根遍历
    ...
    if (T)
    {
        ...
        case EXT_VAR_DEF_ASSIGN:
            ...
            display(T->ptr[0], indent + 3); //显示外部变量类型
            printf("%*c变量名：\n", indent + 3, ' ');
            display(T->ptr[1], indent + 6); //显示变量
            display(T->ptr[2], indent + 6); //显示变量
            break;
        case TYPE:
            printf("%*c类型： %s\n", indent, ' ', T->type_id);
            break;
        ...
        case EXP_ELE:
            break;
        case EXP_ARRAY:
			printf("%*c数组元素：%s",indent, ' ', T->ptr[0]->type_id);
			printf("[ %s ]\n", T->ptr[1]->type_id);
			//display(T->ptr[1],indent+3);
			//printf("%*c数组下标：%s\n",indent, ' ', T->ptr[1]->type_id);
			//display(T->ptr[0],indent);
        ...
        case FOR:
            ...
            break;
        case FOR_DEC:
            display(T->ptr[0], indent + 6);//显示第一个表达式
            display(T->ptr[1], indent + 6);//显示第二个表达式
            display(T->ptr[2], indent + 6);//显示第三个表达式
            break;
        case IF_THEN:
        ...
        case DEC_LIST:
            ...
            while (T0)
            {
                ...
                {
                    printf("%*c %s \n%*cASSIGNOP\n ", indent + 3, ' ', T0->ptr[0]->ptr[0]->type_id, indent + 6, ' ');
                    //显示初始化表达式
                    display(T0->ptr[0]->ptr[1], indent + strlen(T0->ptr[0]->ptr[0]->type_id) + 4);
                }
                else if (T0->ptr[0]->kind == ASSIGNOP_PLUS)
                {
                    printf("%*c %s ASSIGNOP_PLUS\n ", indent + 3, ' ', T0->ptr[0]->ptr[0]->type_id);
                    //显示初始化表达式
                    display(T0->ptr[0]->ptr[1], indent + strlen(T0->ptr[0]->ptr[0]->type_id) + 4);
                }
                else if (T0->ptr[0]->kind == ASSIGNOP_MINUS)
                {
                    printf("%*c %s ASSIGNOP_MINUS\n ", indent + 3, ' ', T0->ptr[0]->ptr[0]->type_id);
                    //显示初始化表达式
                    display(T0->ptr[0]->ptr[1], indent + strlen(T0->ptr[0]->ptr[0]->type_id) + 4);
                }
                else if (T0->ptr[0]->kind == ASSIGNOP_STAR)
                {
                    printf("%*c %s ASSIGNOP_STAR\n ", indent + 3, ' ', T0->ptr[0]->ptr[0]->type_id);
                    //显示初始化表达式
                    display(T0->ptr[0]->ptr[1], indent + strlen(T0->ptr[0]->ptr[0]->type_id) + 4);
                }
                else if (T0->ptr[0]->kind == ASSIGNOP_DIV)
                {
                    printf("%*c %s ASSIGNOP_DIV\n ", indent + 3, ' ', T0->ptr[0]->ptr[0]->type_id);
                    //显示初始化表达式
                    display(T0->ptr[0]->ptr[1], indent + strlen(T0->ptr[0]->ptr[0]->type_id) + 4);
                }
                T0 = T0->ptr[1];
