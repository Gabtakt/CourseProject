...
// 结构体、for循环的非终结符定义
%type  <ptr> StructSpecifier  StructCompSt StructName  StructDec ForDec
...
%token PLUS_SELF MINUS_SELF PLUS MINUS STAR DIV ASSIGNOP ASSIGNOP_PLUS ASSIGNOP_MINUS ASSIGNOP_STAR ASSIGNOP_DIV AND OR NOT VARDEC
...
%left PLUS MINUS
%left STAR DIV
%right UMINUS NOT PLUS_SELF MINUS_SELF
...
ExtDef: Specifier ExtDecList SEMI     {$$=mknode(EXT_VAR_DEF,$1,$2,NULL,yylineno);}   //该结点对应一个外部变量声明
	  ...
	  | StructSpecifier StructCompSt SEMI {$$=mknode(STRUCT_DEF,$1,$2,NULL,yylineno);}       //该节点对应一个结构体定义
      | error SEMI                    {$$=NULL; }
      ;
ExtDecList: VarDec                  {$$=$1;}       /*每一个EXT_DECLIST的结点，其第一棵子树对应一个变量名(ID类型的结点),第二棵子树对应剩下的外部变量名*/
          ...
          ;  
Specifier: TYPE          {$$=mknode(TYPE,NULL,NULL,NULL,yylineno);strcpy($$->type_id,$1);if(!strcmp($1,"int"))$$->type=INT;if(!strcmp($1,"float"))$$->type=FLOAT;if(!strcmp($1,"double"))$$->type=DOUBLE;if(!strcmp($1,"char"))$$->type=CHAR;if(!strcmp($1,"string"))$$->type=STRING;}   //符
         | StructSpecifier {$$=$1}
		  ;      
StructSpecifier: STRUCT StructName {$$=mknode(STRUCT_TYPE,$2,NULL,NULL,yylineno)}

StructName: {$$=NULL;}
          | ID {$$=mknode(STRUCT_ID,NULL,NULL,NULL,yylineno);strcpy($$->type_id,$1);}
          ;
VarDec: ID          {$$=mknode(ID,NULL,NULL,NULL,yylineno);strcpy($$->type_id,$1);}   //ID结点，标识符符号串存放结点的type_id
	  ...
	  | StructDec  {$$=$1;}
      ;
//ArrayDec: ID                   {$$=mknode(ARRAY_ID,NULL,NULL,NULL,yylineno);strcpy($$->type_id,$1);}
//		| ArrayDec LB Exp RB   {$$=mknode(ARRAY_DEP,$1,$3,NULL,yylineno);}
...
CompSt: LC DefList StmList RC    {$$=mknode(COMP_STM,$2,$3,NULL,yylineno);}
      ;
StructCompSt: LC DefList RC      {$$=mknode(STRUCT_COMP_STM,$2,NULL,NULL,yylineno);}
		    ;
StmList: {$$=NULL; }  
...
ForDec: Exp SEMI Exp SEMI Exp {$$=mknode(FOR_DEC,$1,$3,$5,yylineno);}
      ...
      ;
StructDec: StructSpecifier ID  {$$=mknode(STRUCT_DEC,$1,NULL,NULL,yylineno);}
           ;
DefList: {$$=NULL; }
       | Def DefList {$$=mknode(DEF_LIST,$1,$2,NULL,yylineno);}
       ;
Def: Specifier DecList SEMI {$$=mknode(VAR_DEF,$1,$2,NULL,yylineno);}
   | StructSpecifier StructCompSt SEMI {$$=mknode(STRUCT_DEF,$1,$2,NULL,yylineno);}
   ;
...
Exp:    Exp ASSIGNOP Exp {$$=mknode(ASSIGNOP,$1,$3,NULL,yylineno);strcpy($$->type_id,"=");}//$$结点type_id空置未用，正好存放运算符
      ...
      | Exp PLUS_SELF  {$$=mknode(PLUS_SELF,$1,NULL,NULL,yylineno);strcpy($$->type_id,"++");}
      | PLUS_SELF Exp  {$$=mknode(PLUS_SELF,$2,NULL,NULL,yylineno);strcpy($$->type_id,"++");}
      ...
      | Exp MINUS_SELF  {$$=mknode(MINUS_SELF,$1,NULL,NULL,yylineno);strcpy($$->type_id,"--");}
      | MINUS_SELF Exp  {$$=mknode(MINUS_SELF,$2,NULL,NULL,yylineno);strcpy($$->type_id,"--");}
      ...
      | DOUBLE         {$$=mknode(DOUBLE,NULL,NULL,NULL,yylineno);$$->type_float=$1;$$->type=DOUBLE;}
      ...
      | VOID           {$$=mknode(VOID,NULL,NULL,NULL,yylineno);$$->type=VOID;}
	  ;
